...
static int glob(const char *pattern, ATTRIBUTE_UNUSED int flags,
  ...
  if (ret->gl_pathv[ret->gl_pathc] == NULL) goto done;
  ret->gl_pathc++;
  while (FindNextFileA(hFind, &FindFileData)) {
    ...
    strncpy(directory + len, FindFileData.cFileName, 499 - len);
    ret->gl_pathv[ret->gl_pathc] = xmlMemStrdup(directory);
    if (ret->gl_pathv[ret->gl_pathc] == NULL) break;
    ret->gl_pathc++;
  }
...
static void globfree(glob_t *pglob) {
  ...
  for (i = 0; i < pglob->gl_pathc; i++) {
    if (pglob->gl_pathv[i] != NULL) xmlFree(pglob->gl_pathv[i]);
  }
...
static int glob(const char *pattern ATTRIBUTE_UNUSED,
...
static void testErrorHandler(void *ctx ATTRIBUTE_UNUSED, const char *msg, ...) {
  va_list args;
  int res;

  if (testErrorsSize >= 32768) return;
  va_start(args, msg);
  res =
      vsnprintf(&testErrors[testErrorsSize], 32768 - testErrorsSize, msg, args);
  va_end(args);
...
static void testStructuredErrorHandler(void *ctx ATTRIBUTE_UNUSED,
                                       const xmlError *err) {
...
static char *resultFilename(const char *filename, const char *out,
  ...
  if (snprintf(res, 499, "%s%s%s", out, base, suffixbuff) >= 499) res[499] = 0;
...
#if defined(_WIN32)
  if (!(buf.st_mode & _S_IFREG)) return (0);
#else
...
static int compareFiles(const char *r1 /* temp */,
  ...
  if (update_results) {
    ...
    do {
      ...
      if (res2 <= 0 || res2 != res1) break;
    } while (1);
    close(fd2);
    close(fd1);
    if (total == 0) unlink(r2);
    return (res1 != 0);
  }
 ...
 * @param ctxt  An XML parser context
 */
static void internalSubsetDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
                                const xmlChar *ExternalID,
                                const xmlChar *SystemID) {
  callbacks++;
  if (quiet) return;
  if (name == NULL) name = BAD_CAST "(null)";
 ...
 * @param ctxt  An XML parser context
 */
static void externalSubsetDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
                                const xmlChar *ExternalID,
                                const xmlChar *SystemID) {
  callbacks++;
  if (quiet) return;
...
static void entityDeclDebug(void *ctx, const xmlChar *name, int type,
  ...
  if (publicId == NULL) publicId = nullstr;
  if (systemId == NULL) systemId = nullstr;
  if (content == NULL) content = (xmlChar *)nullstr;
  callbacks++;
  if (quiet) return;
 ...
 */
static void attributeDeclDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *elem,
 ...
 */
static void elementDeclDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
                             int type,
 ...
 * @param systemId  The system ID of the entity
 */
static void notationDeclDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
                              const xmlChar *publicId,
                              const xmlChar *systemId) {
  callbacks++;
  if (quiet) return;
...
static void unparsedEntityDeclDebug(void *ctx ATTRIBUTE_UNUSED,
  ...
  if (publicId == NULL) publicId = nullstr;
  if (systemId == NULL) systemId = nullstr;
  if (notationName == NULL) notationName = nullstr;
  callbacks++;
  if (quiet) return;
 ...
 * @param name  The element name
 */
static void startElementDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *name,
                              const xmlChar **atts) {
  int i;
  ...
  if (atts != NULL) {
    for (i = 0; (atts[i] != NULL); i++) {
      ...
      if (atts[i] != NULL) fprintf(SAXdebug, "%s'", atts[i]);
    }
  }
  fprintf(SAXdebug, ")\n");
}
 ...
 */
static void charactersDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *ch,
                            int len) {
  char output[40];
  int i;
  ...
  callbacks++;
  if (quiet) return;
  for (i = 0; (i < len) && (i < 30); i++) output[i] = (char)ch[i];
  output[i] = 0;
 ...
 */
static void cdataBlockDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *value,
                            int len) {
  callbacks++;
  if (quiet) return;
...
static xmlSAXHandler debugSAXHandlerStruct = {internalSubsetDebug,
                                              isStandaloneDebug,
...
static void startElementNsDebug(void *ctx ATTRIBUTE_UNUSED,
  ...
  if (namespaces != NULL) {
    for (i = 0; i < nb_namespaces * 2; i++) {
      ...
      if (namespaces[i] != NULL) fprintf(SAXdebug, ":%s", namespaces[i]);
      i++;
...
static xmlSAXHandler debugSAX2HandlerStruct = {internalSubsetDebug,
                                               isStandaloneDebug,
                                               hasInternalSubsetDebug,
                                               hasExternalSubsetDebug,
                                               resolveEntityDebug,
 ...
 * @param name  The element name
 */
static void htmlstartElementDebug(void *ctx ATTRIBUTE_UNUSED,
 ...
 * @param len  the number of xmlChar
 */
static void htmlcharactersDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *ch,
                                int len) {
  unsigned char output[40];
 ...
 * @param len  the number of xmlChar
 */
static void htmlcdataDebug(void *ctx ATTRIBUTE_UNUSED, const xmlChar *ch,
                           int len) {
  unsigned char output[40];
  int inlen = len, outlen = 30;
...
static xmlSAXHandler debugHTMLSAXHandlerStruct = {internalSubsetDebug,
                                                  isStandaloneDebug,
                                                  hasInternalSubsetDebug,
                                                  hasExternalSubsetDebug,
                                                  resolveEntityDebug,
...
static void hashFreeEntity(void *payload,
 ...
 */
static int saxParseTest(const char *filename, const char *result,
...
static void startDocumentTokenizer(void *ctx) {
...
static void pendingTokenizer(xmlTokenizerConfig *config) {
...
static void internalSubsetTokenizer(void *ctx, const xmlChar *name,
                                    const xmlChar *publicId,
                                    const xmlChar *systemId) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
...
static void startElementTokenizer(void *ctx, const xmlChar *name,
                                  const xmlChar **atts) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
  int i;
  ...
  if (atts != NULL) {
    for (i = 0; atts[i] != NULL; i += 2) {
      fprintf(SAXdebug, " %s=", atts[i]);
      if (atts[i + 1] != NULL) fprintf(SAXdebug, "%s", atts[i + 1]);
    }
  }
  fprintf(SAXdebug, "\n");
}

static void endElementTokenizer(void *ctx, const xmlChar *name) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
...
static void charactersTokenizer(void *ctx, const xmlChar *ch, int len) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
...
static void commentTokenizer(void *ctx, const xmlChar *value) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
...
static void endDocumentTokenizer(void *ctx) {
  xmlParserCtxtPtr ctxt = ctx;
  xmlTokenizerConfig *config = ctxt->_private;
...
static xmlSAXHandler tokenizeHtmlSAXHandler = {internalSubsetTokenizer,
                                               NULL,
                                               NULL,
                                               NULL,
                                               NULL,
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int htmlTokenizerTest(const char *filename, const char *result,
                             const char *err ATTRIBUTE_UNUSED, int options) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int oldParseTest(const char *filename, const char *result,
...
static void internalSubsetBnd(void *ctx, const xmlChar *name,
                              const xmlChar *externalID,
                              const xmlChar *systemID) {
  pushBoundaryCount++;
...
static void referenceBnd(void *ctx, const xmlChar *name) {
  pushBoundaryRefCount++;
  xmlSAX2Reference(ctx, name);
}

static void charactersBnd(void *ctx, const xmlChar *ch, int len) {
  pushBoundaryCount++;
  pushBoundaryCharsCount++;
  xmlSAX2Characters(ctx, ch, len);
}

static void cdataBlockBnd(void *ctx, const xmlChar *ch, int len) {
  pushBoundaryCount++;
  pushBoundaryCDataCount++;
  xmlSAX2CDataBlock(ctx, ch, len);
}

static void processingInstructionBnd(void *ctx, const xmlChar *target,
                                     const xmlChar *data) {
  pushBoundaryCount++;
  xmlSAX2ProcessingInstruction(ctx, target, data);
}

static void commentBnd(void *ctx, const xmlChar *value) {
  xmlParserCtxtPtr ctxt = ctx;
  if (ctxt->inSubset == 0) pushBoundaryCount++;
  xmlSAX2Comment(ctx, value);
}

static void startElementBnd(void *ctx, const xmlChar *xname,
                            const xmlChar **atts) {
  const char *name = (const char *)xname;

  /* Some elements might be created automatically. */
  if ((strcmp(name, "html") != 0) && (strcmp(name, "body") != 0) &&
      (strcmp(name, "head") != 0) && (strcmp(name, "p") != 0)) {
    pushBoundaryCount++;
  }
...
static void startElementNsBnd(void *ctx, const xmlChar *localname,
...
static void endElementNsBnd(void *ctx, const xmlChar *localname,
                            const xmlChar *prefix, const xmlChar *URI) {
  /*pushBoundaryCount++;*/
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int pushBoundaryTest(const char *filename, const char *result,
                            const char *err ATTRIBUTE_UNUSED, int options) {
  xmlParserCtxtPtr ctxt;
  xmlDocPtr doc;
   ...
   */
  if (strcmp(filename, "./test/HTML/xml-declaration-1.html") == 0) return (0);
  ...
  while ((cur < size) && (numCallbacks <= 1) && (avail <= 0)) {
    ...
    if (ctxt->instate == XML_PARSER_CONTENT) {
      int firstChar =
          (ctxt->input->end > ctxt->input->cur) ? *ctxt->input->cur : base[cur];
    ...
    consumed = ctxt->input->consumed +
               (unsigned long)(ctxt->input->cur - ctxt->input->base);
    if ((ctxt->instate != XML_PARSER_DTD) && (consumed >= 4) &&
        (consumed != oldConsumed)) {
      size_t max = 0;

      avail = ctxt->input->end - ctxt->input->cur;

      if ((options & XML_PARSE_HTML) && (ctxt->instate == XML_PARSER_END_TAG)) {
        /* Something related to script parsing. */
        max = 3;
      } else if (isText) {
        int c = *ctxt->input->cur;

        if ((options & XML_PARSE_HTML) && (ctxt->endCheckState)) {
...
static char *dumpNodeList(xmlNodePtr list) {
  xmlBufferPtr buffer;
  xmlSaveCtxtPtr save;
  xmlNodePtr cur;
  char *ret;
  ...
  for (cur = list; cur != NULL; cur = cur->next) xmlSaveTree(save, cur);
  xmlSaveClose(save);
...
static int testParseContent(xmlParserCtxtPtr ctxt, xmlDocPtr doc,
                            const char *filename) {
   ...
   */
  if (strcmp(filename, "./test/undeclared-entity.xml") == 0) return 0;

  if (ctxt->html) {
    xmlNodePtr cur;

    if (doc == NULL || doc->children == NULL) return 0;
  ...
  xmlParseInNodeContext(root, content, strlen(content),
                        ctxt->options | XML_PARSE_NOERROR, &list);
  roundTrip = dumpNodeList(list);
  if (strcmp(content, roundTrip) != 0) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int memParseTest(const char *filename, const char *result,
                        const char *err ATTRIBUTE_UNUSED, int options) {
  xmlParserCtxtPtr ctxt;
  xmlDocPtr doc;
  const char *base;
  ...
  if (testParseContent(ctxt, doc, filename) < 0) ret = -1;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int noentParseTest(const char *filename, const char *result,
                          const char *err ATTRIBUTE_UNUSED, int options) {
  xmlDocPtr doc;
  char *temp;
  int res = 0;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int errParseTest(const char *filename, const char *result,
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int fdParseTest(const char *filename, const char *result,
                       const char *err, int options) {
  xmlParserCtxtPtr ctxt;
  xmlDocPtr doc;
...
static void processNode(FILE *out, xmlTextReaderPtr reader) {
  ...
  fprintf(out, "%d %d %s %d %d", xmlTextReaderDepth(reader), type, name, empty,
...
static int streamProcessTest(const char *filename, const char *result,
                             const char *err, xmlTextReaderPtr reader,
                             const char *rng, int options ATTRIBUTE_UNUSED) {
  int ret;
  char *temp = NULL;
  FILE *t = NULL;
  ...
  while (ret == 1) {
    if ((t != NULL) && (rng == NULL)) processNode(t, reader);
    ret = xmlTextReaderRead(reader);
  }
  if (ret != 0) {
    testErrorHandler(NULL, "%s : failed to parse\n", filename);
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int streamParseTest(const char *filename, const char *result,
                           const char *err, int options) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int walkerParseTest(const char *filename, const char *result,
                           const char *err, int options) {
  xmlDocPtr doc;
  xmlTextReaderPtr reader;
  int ret;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int streamMemParseTest(const char *filename, const char *result,
                              const char *err, int options) {
  xmlTextReaderPtr reader;
  int ret;
  const char *base;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int xpathCommonTest(const char *filename, const char *result, int xptr,
                           int expr) {
  FILE *input;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int xpathExprTest(const char *filename, const char *result,
                         const char *err ATTRIBUTE_UNUSED,
                         int options ATTRIBUTE_UNUSED) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int xpathDocTest(const char *filename,
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int xptrDocTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
                       const char *err ATTRIBUTE_UNUSED,
                       int options ATTRIBUTE_UNUSED) {
  char pattern[500];
  char result[500];
  ...
  xpathDocument =
      xmlReadFile(filename, NULL, XML_PARSE_DTDATTR | XML_PARSE_NOENT);
  if (xpathDocument == NULL) {
    fprintf(stderr, "Failed to load %s\n", filename);
    return (-1);
  }

  res = snprintf(pattern, 499, "./test/XPath/xptr/%s*", baseFilename(filename));
  if (res >= 499) pattern[499] = 0;
  globbuf.gl_offs = 0;
  glob(pattern, GLOB_DOOFFS, NULL, &globbuf);
  for (i = 0; i < globbuf.gl_pathc; i++) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int xmlidDocTest(const char *filename, const char *result,
                        const char *err, int options) {
  xmlParserCtxtPtr ctxt;
  int res = 0;
  int ret = 0;
  ...
  ctxt = xmlNewParserCtxt();
  xmlCtxtSetErrorHandler(ctxt, testStructuredErrorHandler, NULL);
  xpathDocument = xmlCtxtReadFile(
      ctxt, filename, NULL, options | XML_PARSE_DTDATTR | XML_PARSE_NOENT);
  xmlFreeParserCtxt(ctxt);
  if (xpathDocument == NULL) {
    fprintf(stderr, "Failed to load %s\n", filename);
    return (-1);
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int uriCommonTest(const char *filename, const char *result,
                         const char *err, const char *base) {
  char *temp;
  FILE *o, *f;
  char str[1024];
  int res = 0, i, ret;
  ...
  while (1) {
     ...
     */
    if (fgets(&str[0], sizeof(str) - 1, f) == NULL) break;
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int uriParseTest(const char *filename, const char *result,
                        const char *err, int options ATTRIBUTE_UNUSED) {
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int uriBaseTest(const char *filename, const char *result,
                       const char *err, int options ATTRIBUTE_UNUSED) {
...
static const char *urip_rcvsURLs[] = {
    ...
    "urip://example.com/test?a=1&b=2%263&c=4#foo", NULL};
...
#ifdef LIBXML_CATALOG_ENABLED
  if ((URI == NULL) || (!strcmp(URI, "file://" XML_SYSCONFDIR "/xml/catalog")))
    return (0);
#endif
  /* Verify we received the escaped URL */
  if (strcmp(urip_rcvsURLs[urip_current], URI)) urip_success = 0;
  return (1);
}
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int uriPathTest(const char *filename ATTRIBUTE_UNUSED,
...
#ifdef LIBXML_SCHEMAS_ENABLED
static int schemasOneTest(const char *sch, const char *filename,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (schemas == NULL) goto done;

    ctxt = xmlSchemaNewValidCtxt(schemas);
    xmlSchemaSetValidStructuredErrors(ctxt, testStructuredErrorHandler, NULL);
    ...
    if (validResult == 0) {
      ...
      testErrorHandler(NULL,
                       "%s validation generated an internal "
                       "error\n",
                       filename);
    }
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int schemasTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
                       const char *errr ATTRIBUTE_UNUSED, int options) {
...
#ifdef LIBXML_RELAXNG_ENABLED
static int rngOneTest(const char *sch, const char *filename, int options,
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int rngTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
                   const char *errr ATTRIBUTE_UNUSED, int options) {
  const char *base = baseFilename(filename);
  const char *base2;
  const char *instance;
  ...
  xmlRelaxNGSetParserStructuredErrors(ctxt, testStructuredErrorHandler, NULL);
  ...
  for (i = 0; i < globbuf.gl_pathc; i++) {
    ...
    if ((len > 6) && (base2[len - 6] == '_')) {
      count = base2[len - 5];
      res = snprintf(err, 499, "result/relaxng/%s_%c.err", prefix, count);
      if (res >= 499) err[499] = 0;
    } else {
      fprintf(stderr, "don't know how to process %s\n", instance);
      continue;
    }
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int rngStreamTest(const char *filename,
                         const char *resul ATTRIBUTE_UNUSED,
                         const char *errr ATTRIBUTE_UNUSED, int options) {
  const char *base = baseFilename(filename);
  const char *base2;
  ...
  for (i = 0; i < globbuf.gl_pathc; i++) {
    ...
    if ((len > 6) && (base2[len - 6] == '_')) {
      count = base2[len - 5];
      ret = snprintf(result, 499, "result/relaxng/%s_%c", prefix, count);
      if (ret >= 499) result[499] = 0;
      ret = snprintf(err, 499, "result/relaxng/%s_%c.err", prefix, count);
      if (ret >= 499) err[499] = 0;
    } else {
      fprintf(stderr, "don't know how to process %s\n", instance);
      continue;
    ...
    if (disable_err == 1)
      ret =
          streamProcessTest(instance, result, NULL, reader, filename, options);
    else
...
#ifdef LIBXML_SCHEMATRON_ENABLED
static int schematronOneTest(const char *sch, const char *filename, int options,
  ...
  xmlSchematronSetValidStructuredErrors(ctxt, testStructuredErrorHandler, NULL);
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int schematronTest(const char *filename,
                          const char *resul ATTRIBUTE_UNUSED,
                          const char *errr ATTRIBUTE_UNUSED, int options) {
  const char *base = baseFilename(filename);
  const char *base2;
  ...
  for (i = 0; i < globbuf.gl_pathc; i++) {
    ...
    if ((len > 6) && (base2[len - 6] == '_')) {
      count = base2[len - 5];
      res = snprintf(err, 499, "result/schematron/%s_%c.err", prefix, count);
      if (res >= 499) err[499] = 0;
    } else {
      fprintf(stderr, "don't know how to process %s\n", instance);
      continue;
 ...
 *									*
 ************************************************************************/
static void patternNode(FILE *out, xmlTextReaderPtr reader, const char *pattern,
                        xmlPatternPtr patternc, xmlStreamCtxtPtr patstream) {
  xmlChar *path = NULL;
  int match = -1;
  int type, empty;
  ...
  if (patstream != NULL) {
    ...
    if (type == XML_READER_TYPE_ELEMENT) {
      ret = xmlStreamPush(patstream, xmlTextReaderConstLocalName(reader),
      ...
      if (ret < 0) {
        ...
        fprintf(out, "xmlPatternMatch and xmlStreamPush disagree\n");
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int patternTest(const char *filename, const char *resul ATTRIBUTE_UNUSED,
                       const char *err ATTRIBUTE_UNUSED, int options) {
  ...
  while (1) {
    ...
    if (doc == NULL) {
      ...
      patternc =
          xmlPatterncompile((const xmlChar *)str, doc->dict, 0, &namespaces[0]);
      if (patternc == NULL) {
        testErrorHandler(NULL, "Pattern %s failed to compile\n", str);
        xmlFreeDoc(doc);
        ret = 1;
        continue;
      }
 ...
 *									*
 ************************************************************************/
static xmlXPathObjectPtr load_xpath_expr(xmlDocPtr parent_doc,
                                         const char *filename) {
...
static int c14nRunTest(const char *xml_filename, int with_comments, int mode,
   ...
   */
  ret = xmlC14NDocDumpMemory(doc, (xpath) ? xpath->nodesetval : NULL, mode,
                             inclusive_namespaces, with_comments, &result);
  if (ret >= 0) {
    if (result != NULL) {
...
static int c14nCommonTest(const char *filename, int with_comments, int mode,
  ...
  nb_tests++;
  if (c14nRunTest(filename, with_comments, mode, xpath, ns, result) < 0)
    ret = 1;
...
static int c14nWithoutCommentTest(const char *filename,
                                  const char *resul ATTRIBUTE_UNUSED,
                                  const char *err ATTRIBUTE_UNUSED,
                                  int options ATTRIBUTE_UNUSED) {
  return (c14nCommonTest(filename, 0, XML_C14N_1_0, "without-comments"));
}
static int c14nExcWithoutCommentTest(const char *filename,
                                     const char *resul ATTRIBUTE_UNUSED,
                                     const char *err ATTRIBUTE_UNUSED,
                                     int options ATTRIBUTE_UNUSED) {
  return (c14nCommonTest(filename, 0, XML_C14N_EXCLUSIVE_1_0,
                         "exc-without-comments"));
}
static int c14n11WithoutCommentTest(const char *filename,
                                    const char *resul ATTRIBUTE_UNUSED,
                                    const char *err ATTRIBUTE_UNUSED,
                                    int options ATTRIBUTE_UNUSED) {
...
static xmlThreadParams threadParams[] = {
    {"test/threads/abc.xml", 0},    {"test/threads/acb.xml", 0},
    {"test/threads/bac.xml", 0},    {"test/threads/bca.xml", 0},
    {"test/threads/cab.xml", 0},    {"test/threads/cba.xml", 0},
    {"test/threads/invalid.xml", 0}};
static const unsigned int num_threads =
    sizeof(threadParams) / sizeof(threadParams[0]);

static void *thread_specific_data(void *private_data) {
  xmlDocPtr myDoc;
...
static DWORD WINAPI win32_thread_specific_data(void *private_data) {
...
static int testThread(void) {
  ...
  for (repeat = 0; repeat < TEST_REPEAT_COUNT; repeat++) {
    ...
    for (i = 0; i < num_threads; i++) {
      ...
      tid[i] = CreateThread(NULL, 0, win32_thread_specific_data,
                            (void *)&threadParams[i], 0, &useless);
      if (tid[i] == NULL) {
...
#endif
static int threadsTest(const char *filename ATTRIBUTE_UNUSED,
                       const char *resul ATTRIBUTE_UNUSED,
                       const char *err ATTRIBUTE_UNUSED,
                       int options ATTRIBUTE_UNUSED) {
  return (testThread());
...
static int regexpTest(const char *filename, const char *result, const char *err,
                      int options ATTRIBUTE_UNUSED) {
  ...
  fclose(output);
  fclose(input);
  if (comp != NULL) xmlRegFreeRegexp(comp);
...
static int automataTest(const char *filename, const char *result,
                        const char *err ATTRIBUTE_UNUSED,
                        int options ATTRIBUTE_UNUSED) {
  FILE *input, *output;
  char *temp;
  ...
  for (i = 0; i < 1000; i++) states[i] = NULL;
  ...
  while (fgets(expr, 4500, input) != NULL) {
    ...
    if (len >= 0) {
      if ((am != NULL) && (expr[0] == 't') && (expr[1] == ' ')) {
        ...
        if (states[from] == NULL) states[from] = xmlAutomataNewState(am);
        ptr++;
        to = scanNumber(&ptr);
        if (*ptr != ' ') {
          fprintf(stderr, "Bad line %s\n", expr);
        ...
        if (states[to] == NULL) states[to] = xmlAutomataNewState(am);
        ptr++;
        xmlAutomataNewTransition(am, states[from], states[to], BAD_CAST ptr,
                                 NULL);
        ...
        ptr++;
        xmlAutomataNewCountTrans(am, states[from], states[to], BAD_CAST ptr,
                                 min, max, NULL);
        ...
        am = NULL;
        if (regexp == NULL) {
          fprintf(stderr, "Failed to compile the automata");
          break;
        }
        ...
        if (regexp == NULL) {
          ...
          if (exec == NULL) exec = xmlRegNewExecCtxt(regexp, NULL, NULL);
          if (ret == 0) {
  ...
  fclose(output);
  fclose(input);
  if (regexp != NULL) xmlRegFreeRegexp(regexp);
  if (exec != NULL) xmlRegFreeExecCtxt(exec);
  if (am != NULL) xmlFreeAutomata(am);
...
static testDesc testDescriptions[] = {
    {"XML regression tests", oldParseTest, "./test/*", "result/", "", NULL, 0},
    {"XML regression tests on memory", memParseTest, "./test/*", "result/", "",
     NULL, 0},
    {"XML entity subst regression tests", noentParseTest, "./test/*",
     "result/noent/", "", NULL, XML_PARSE_NOENT},
    {"XML Namespaces regression tests", errParseTest, "./test/namespaces/*",
     "result/namespaces/", "", ".err", 0},
#ifdef LIBXML_VALID_ENABLED
    {"Error cases regression tests", errParseTest, "./test/errors/*.xml",
     "result/errors/", "", ".err", 0},
    {"Error cases regression tests from file descriptor", fdParseTest,
     "./test/errors/*.xml", "result/errors/", "", ".err", 0},
    {"Error cases regression tests with entity substitution", errParseTest,
     "./test/errors/*.xml", "result/errors/", NULL, ".ent", XML_PARSE_NOENT},
    {"Error cases regression tests (old 1.0)", errParseTest,
     "./test/errors10/*.xml", "result/errors10/", "", ".err", XML_PARSE_OLD10},
#endif
#ifdef LIBXML_READER_ENABLED
#ifdef LIBXML_VALID_ENABLED
    {"Error cases stream regression tests", streamParseTest,
     "./test/errors/*.xml", "result/errors/", NULL, ".str", 0},
#endif
    {"Reader regression tests", streamParseTest, "./test/*", "result/", ".rdr",
     NULL, 0},
    {"Reader entities substitution regression tests", streamParseTest,
     "./test/*", "result/", ".rde", NULL, XML_PARSE_NOENT},
    {"Reader on memory regression tests", streamMemParseTest, "./test/*",
     "result/", ".rdr", NULL, 0},
    {"Walker regression tests", walkerParseTest, "./test/*", "result/", ".rdr",
     NULL, 0},
#endif
#ifdef LIBXML_SAX1_ENABLED
    {"SAX1 callbacks regression tests", saxParseTest, "./test/*", "result/",
     ".sax", NULL, XML_PARSE_SAX1},
#endif
    {"SAX2 callbacks regression tests", saxParseTest, "./test/*", "result/",
     ".sax2", NULL, 0},
    {"SAX2 callbacks regression tests with entity substitution", saxParseTest,
     "./test/*", "result/noent/", ".sax2", NULL, XML_PARSE_NOENT},
#ifdef LIBXML_PUSH_ENABLED
    {"XML push regression tests", pushParseTest, "./test/*", "result/", "",
     NULL, 0},
    {"XML push boundary tests", pushBoundaryTest, "./test/*", "result/", "",
     NULL, 0},
#endif
#ifdef LIBXML_HTML_ENABLED
    {"HTML regression tests", errParseTest, "./test/HTML/*", "result/HTML/", "",
     ".err", XML_PARSE_HTML},
    {"HTML regression tests from file descriptor", fdParseTest, "./test/HTML/*",
     "result/HTML/", "", ".err", XML_PARSE_HTML},
#ifdef LIBXML_PUSH_ENABLED
    {"Push HTML regression tests", pushParseTest, "./test/HTML/*",
     "result/HTML/", "", ".err", XML_PARSE_HTML},
    {"Push HTML boundary tests", pushBoundaryTest, "./test/HTML/*",
     "result/HTML/", "", NULL, XML_PARSE_HTML},
#endif
    {"HTML SAX regression tests", saxParseTest, "./test/HTML/*", "result/HTML/",
     ".sax", NULL, XML_PARSE_HTML},
#ifdef LIBXML_PUSH_ENABLED
    {"HTML tokenization tests", htmlTokenizerTest,
     "./test/html-tokenizer/*.test", "result/html-tokenizer/", "", NULL, 0},
#endif
#endif
#ifdef LIBXML_VALID_ENABLED
    {"Valid documents regression tests", errParseTest, "./test/VCM/*", NULL,
     NULL, NULL, XML_PARSE_DTDVALID},
    {"Validity checking regression tests", errParseTest, "./test/VC/*",
     "result/VC/", NULL, "", XML_PARSE_DTDVALID},
#ifdef LIBXML_READER_ENABLED
    {"Streaming validity checking regression tests", streamParseTest,
     "./test/valid/*.xml", "result/valid/", NULL, ".err.rdr",
     XML_PARSE_DTDVALID},
    {"Streaming validity error checking regression tests", streamParseTest,
     "./test/VC/*", "result/VC/", NULL, ".rdr", XML_PARSE_DTDVALID},
#endif
    {"General documents valid regression tests", errParseTest, "./test/valid/*",
     "result/valid/", "", ".err", XML_PARSE_DTDVALID},
#endif
#ifdef LIBXML_XINCLUDE_ENABLED
    {"XInclude regression tests", errParseTest, "./test/XInclude/docs/*",
     "result/XInclude/", "", ".err", XML_PARSE_XINCLUDE},
#ifdef LIBXML_READER_ENABLED
    {"XInclude xmlReader regression tests", streamParseTest,
     "./test/XInclude/docs/*", "result/XInclude/", ".rdr", ".err",
     XML_PARSE_XINCLUDE},
#endif
    {"XInclude regression tests stripping include nodes", errParseTest,
     "./test/XInclude/docs/*", "result/XInclude/", "", ".err",
...
#endif
    {"XInclude regression tests without reader", errParseTest,
     "./test/XInclude/without-reader/*", "result/XInclude/", "", ".err",
     XML_PARSE_XINCLUDE},
#endif
#ifdef LIBXML_XPATH_ENABLED
#ifdef LIBXML_DEBUG_ENABLED
    {"XPath expressions regression tests", xpathExprTest, "./test/XPath/expr/*",
     "result/XPath/expr/", "", NULL, 0},
    {"XPath document queries regression tests", xpathDocTest,
     "./test/XPath/docs/*", NULL, NULL, NULL, 0},
#ifdef LIBXML_XPTR_ENABLED
    {"XPointer document queries regression tests", xptrDocTest,
     "./test/XPath/docs/*", NULL, NULL, NULL, 0},
#endif
#ifdef LIBXML_VALID_ENABLED
    {"xml:id regression tests", xmlidDocTest, "./test/xmlid/*", "result/xmlid/",
     "", ".err", 0},
#endif
#endif
#endif
    {"URI parsing tests", uriParseTest, "./test/URI/*.uri", "result/URI/", "",
     NULL, 0},
    {"URI base composition tests", uriBaseTest, "./test/URI/*.data",
     "result/URI/", "", NULL, 0},
    {"Path URI conversion tests", uriPathTest, NULL, NULL, NULL, NULL, 0},
#ifdef LIBXML_SCHEMAS_ENABLED
    {"Schemas regression tests", schemasTest, "./test/schemas/*_*.xsd", NULL,
     NULL, NULL, 0},
#endif
#ifdef LIBXML_RELAXNG_ENABLED
    {"Relax-NG regression tests", rngTest, "./test/relaxng/*.rng", NULL, NULL,
     NULL, XML_PARSE_DTDATTR | XML_PARSE_NOENT},
#ifdef LIBXML_READER_ENABLED
    {"Relax-NG streaming regression tests", rngStreamTest,
     "./test/relaxng/*.rng", NULL, NULL, NULL,
     XML_PARSE_DTDATTR | XML_PARSE_NOENT},
#endif
#endif
#if defined(LIBXML_SCHEMATRON_ENABLED)
    {"Schematron regression tests", schematronTest, "./test/schematron/*.sct",
     NULL, NULL, NULL, 0},
#endif
#ifdef LIBXML_PATTERN_ENABLED
#ifdef LIBXML_READER_ENABLED
    {"Pattern regression tests", patternTest, "./test/pattern/*.pat",
     "result/pattern/", NULL, NULL, 0},
#endif
#endif
#ifdef LIBXML_C14N_ENABLED
    {"C14N with comments regression tests", c14nWithCommentTest,
     "./test/c14n/with-comments/*.xml", NULL, NULL, NULL, 0},
    {"C14N without comments regression tests", c14nWithoutCommentTest,
     "./test/c14n/without-comments/*.xml", NULL, NULL, NULL, 0},
    {"C14N exclusive without comments regression tests",
     c14nExcWithoutCommentTest, "./test/c14n/exc-without-comments/*.xml", NULL,
     NULL, NULL, 0},
    {"C14N 1.1 without comments regression tests", c14n11WithoutCommentTest,
     "./test/c14n/1-1-without-comments/*.xml", NULL, NULL, NULL, 0},
#endif
#if defined(LIBXML_THREAD_ENABLED) && defined(LIBXML_CATALOG_ENABLED)
    {"Catalog and Threads regression tests", threadsTest, NULL, NULL, NULL,
     NULL, 0},
#endif
    {"SVG parsing regression tests", oldParseTest, "./test/SVG/*.xml",
     "result/SVG/", "", NULL, 0},
#if defined(LIBXML_REGEXP_ENABLED)
    {"Regexp regression tests", regexpTest, "./test/regexp/*", "result/regexp/",
     "", ".err", 0},
    {"Automata regression tests", automataTest, "./test/automata/*",
     "result/automata/", "", NULL, 0},
#endif
...
static int launchTests(testDescPtr tst) {
  ...
  if (tst->in != NULL) {
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
      if (!checkTestFile(globbuf.gl_pathv[i])) continue;
    ...
    !defined(LIBXML_ISO8859X_ENABLED)
      if (strstr(globbuf.gl_pathv[i], "iso-8859-5") != NULL) continue;
#endif
      if (tst->suffix != NULL) {
